...
//					pbm->skinLocalRotation.normalize();
	...
	for(i=0; i<dat29Suu; ++i) {
		...
		if( i==0 ) {
			...
			pbm->skinLocalMat = glm::row(pbm->skinLocalMat, 3, glm::vec4(pbm->jointLocalTranslation,1));
		}
		else {
			...
			pbm->skinLocalMat = glm::row(pbm->skinLocalMat, 3, glm::vec4(pbm->skinLocalTranslation,1));
		}
	}
	return fr;
}
	...
	for(int i=0; i<size; ++i) {
		...
		if( bpv.x2.x==0 ) {
			...
			if(!m_useQuaternion) {
				...
				B = glm::row(B, 3, glm::vec4(dummy,0));
				h1 = vec3(B*vec4(h1,0));
			}
			else {
