...
//	core::quaternion bindPosRotation;		//jointLocal multiply with parent
//	core::vector3df bindPosTranslation;		//jointLocal multiply with parent
...
/*
Every FFXIPart is created when encounter addVertex
each part can have multiple face. (on encounter TriangleList / TriangleStrip)
each face is an array of indices & uv
each face can reference different texture
each face generate a meshBuffer
...
public:
	...
	bool SetBone(char *p,int sz) {
		...
		for( int ii=0; ii<dat29Suu; ii++ ){
			...
			//glm is column major ==> [x][y] where x is column and y is row
			//therefore the translation is at row 4
			pbm->jointLocalMat = glm::row(pbm->jointLocalMat, 3, glm::vec4(v,1));
			pbm->term=bn.term;
			if( ii>0 ) {
...
//		glm::f32quat qCong = glm::inverse(q);
		// q * v * qConjugate
